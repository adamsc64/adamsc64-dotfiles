#!/bin/bash
# Script to install/symlink custom scripts to ~/scripts directory
set -Eeu -o pipefail

# Python virtual environment
PYTHON_ENV_BASE="$HOME/.venvs"
PYTHON_ENV_PATH="$PYTHON_ENV_BASE/env3"

# Get the directory where this script is located
THIS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Define the destination directory for scripts
SCRIPTS_BASE="$HOME/scripts"
# Create the scripts directory if it doesn't exist
mkdir -p "$SCRIPTS_BASE"

# Array of specific scripts to install
scripts=(
    cdd
    chrome-killall
    f
    get-transcript
    gm
    img2say
    ocr-robust
    pulls
    random
    tail-vscode-logs
    trim
    venv_match.py
    # executable .py and .sh scripts will be added dynamically
)

add_executable_scripts() {
    local ext=$1
    echo "Finding and adding executable '.${ext}' scripts..."
    local pattern="$THIS_DIR/scripts/*.$ext"
    for file in $pattern; do
        [[ -f "$file" && -x "$file" ]] || continue
        local name
        name=$(basename "$file")
        scripts+=("$name")
    done
}

link_script() {
    local script=$1
    local srcpath="${THIS_DIR}/scripts/${script}"
    local destpath="${SCRIPTS_BASE}/${script}"

    if [[ ! -f "$srcpath" ]]; then
        echo "Warning: Source file ${srcpath} does not exist, skipping"
        return
    fi
    if [[ ! -e "$destpath" && ! -L "$destpath" ]]; then
        echo "Creating symlink from ${srcpath} to ${destpath}..."
        ln -s "$srcpath" "$destpath"
    fi
}

_wrap_script_common() {
    local script=$1
    local srcpath="${THIS_DIR}/scripts/${script}"
    local wrapper_name="${script%.*}"  # Remove extension
    wrapper_name="${wrapper_name//_/-}"  # Replace underscores with dashes
    local destpath="${SCRIPTS_BASE}/${wrapper_name}"

    if [[ ! -f "$srcpath" ]]; then
        echo "Warning: Source file ${srcpath} does not exist, skipping"
        return 1
    fi
    if [[ -L "$destpath" ]]; then
        echo "Removing existing symlink ${destpath}..."
        unlink "$destpath"
    fi
    echo "$srcpath:$destpath:$wrapper_name"
}

wrap_py_script() {
    local script=$1
    local parts
    parts=$(_wrap_script_common "$script") || return
    local srcpath destpath wrapper_name
    IFS=: read -r srcpath destpath wrapper_name <<< "$parts"
    if [[ ! -f "$destpath" ]]; then
        echo "Creating wrapper ${wrapper_name} for ${script}..."
        cat > "$destpath" << EOF
#!/bin/bash
# Wrapper script automatically generated by install_scripts.sh
source ${PYTHON_ENV_PATH}/bin/activate
python "${srcpath}" "\$@"
EOF
        chmod +x "$destpath"
    fi
}

wrap_sh_script() {
    local script=$1
    local parts
    parts=$(_wrap_script_common "$script") || return
    local srcpath destpath wrapper_name
    IFS=: read -r srcpath destpath wrapper_name <<< "$parts"
    if [[ ! -f "$destpath" ]]; then
        echo "Creating wrapper ${wrapper_name} for ${script}..."
        cat > "$destpath" << EOF
#!/bin/bash
# Wrapper script automatically generated by install_scripts.sh
exec bash "${srcpath}" "\$@"
EOF
        chmod +x "$destpath"
    fi
}

main() {
    add_executable_scripts py
    add_executable_scripts sh
    echo "Linking and wrapping scripts..."
    for script in "${scripts[@]}"; do
        link_script "$script"
        if [[ "$script" == *.py ]]; then
            wrap_py_script "$script"
        elif [[ "$script" == *.sh ]]; then
            wrap_sh_script "$script"
        fi
    done
}

main "$@"
